...
class ProductPage {
    ...
    async init() {
        const placeholder = "../../img/productplaceholder.jpg" //Placeholder img
        ...
        .then(data => {
            ...
            for (let i = 0; i < product.length; i++){ //For loop of the json
                if (product[i]._id == cleanString) { //Find product id that matches this url id
                    let voorraad = product[i].quantityInStock;
                    ...
                    let avaragereview = totalrating / product[i].reviews.length;
                    ...
                    //Fullstar
                    let nostar = 5 - avaragereview;
                    let star = Math.floor(avaragereview)
